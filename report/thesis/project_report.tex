...
Before we get to our intelligent agents, we must first construct an environment where the agent can receive input -- or state, rather -- and perform actions. For the sake of simplicity this environment will be constrained to the streets surrounding University College London's Bloomsbury Campus. This area provides scope sufficient for the agents to train well and generalise to other situations.

\section{University College London}
OpenStreetMap is a map database maintained by a community of volunteers. Using their data obtained from satellite imagery as well as community submissions, we can extract a portion of University College London's Bloomsbury Campus.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{ucl_bloomsbury_campus.png}
    \caption{Map of University College London's Bloomsbury Campus extracted from OpenStreetMap data.}
\end{figure}
