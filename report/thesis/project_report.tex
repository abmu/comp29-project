...
Where \( Np \) equals the insertion rate calculated from the \texttt{--insertion-density} parameter. Thus, the overall expected value of departures per time step still remains equal to the deterministic insertion rate.

[... image comparing regular interval vs. binomial adjusted insertions]
...
Unfortunately, whilst there isn't a problem with the Poisson distribution per se, the script is designed such that it is only compatible with the prior traffic generation method and certain parameters. Binomial sampling on the other hand is compatible with the latter method and other parameters which are somewhat more flexible and have less implementation-specific issues. For this reason, it was chosen.
